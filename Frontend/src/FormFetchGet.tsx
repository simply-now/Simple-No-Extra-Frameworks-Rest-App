...
export const Form = () => {
    ...
    const onButtonPress = (event: React.FormEvent) => {
        ...
        let int1 = 0;
        let int2 = 0;
        if (formData.firstNumber) {
            int1 = formData.firstNumber; 
        }
        if (formData.secondNumber) {
            int2 = formData.secondNumber; 
        }
        let fetchString = "/simpleapi/?int1=" + int1.toString() + "&int2=" + int2.toString();
        console.log("Sending Get Request:" + fetchString);
        console.log(int1.toString() + "+" + int1.toString() + "=");
        ...
        .then(result => setFormData({type:"UPDATE_TOTAL", payLoad:parseInt(result)}))
        .then(() => console.log(formData.total))
